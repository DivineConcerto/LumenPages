...
const releaseStyles = new Set([
  ...
  'prism', 'aurora', 'editorial', 'cinema', 'atlas', 'lattice', 'nebula', 'glass', 'timeline', 'spotlight',
  'monolith', 'garden', 'blueprint', 'liquid', 'constellation', 'gallery', 'horizon', 'capsule', 'origami', 'equation'
]);

function normalizeReleaseStyle(value = 'orbit') {
  const style = String(value || '').trim();
  if (style === 'mosaic') return 'orbit';
  return releaseStyles.has(style) ? style : 'orbit';
}
...
function seedDatabase() {
  ...
  if (!siteRow) {
    db.prepare(`
      INSERT INTO site_settings (
        ...
        1, @site_name, @site_tagline, @site_description, @hero_badge, @hero_title, @hero_subtitle,
        @about_title, @about_body, @game_badge, @game_title, @game_subtitle, @game_body, @game_cta_label,
        @footer_note, @email, @location, @x_url, @github_url, @linkedin_url, @nav_items, @updated_at
      )
    `).run(defaultSite);
  ...
  if (releaseCount === 0) {
    ...
    `).all();
    const styles = ['ribbon', 'stack', 'console', 'mosaic'];
...
function normalizeReleaseInput(payload, existing = null) {
  ...
  const introduction = String(payload.introduction || existing?.introduction || '').trim();
  if (!introduction) {
    throw new Error('发布项介绍不能为空。');
  }

  const targetUrl = String(payload.target_url || existing?.target_url || '').trim();
  if (!targetUrl) {
    throw new Error('跳转 URL 不能为空。');
  }

  const style = normalizeReleaseStyle(payload.visual_style || existing?.visual_style || 'orbit');
  ...
  return {
    eyebrow: String(payload.eyebrow || existing?.eyebrow || 'Release').trim() || 'Release',
    title,
    introduction,
    target_url: targetUrl,
    cta_label: String(payload.cta_label || existing?.cta_label || '打开').trim() || '打开',
    visual_style: style,
